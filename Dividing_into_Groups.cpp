...
bool checker_boy(vector<int> &arr, ll midtermsexam, ll sz, ll kk)
{
    ll c = 0;
    fl(sz)
    {
        int x = min(midtermsexam, (int)arr[i]);
        c += x;
    }
    return c >= kk * midtermsexam;
}
void solve()
{
    bool flagg = false;
    ...
    fl(n)
        upper_bound += arr[i];
    int low = 0, high = upper_bound, mid = 0;
    while (high > 1 + low)
    {
        mid = (high + low) / 2;
        if (checker_boy(arr, mid, n, k) == 0)
        {
            high = mid - 1;
        }
        else
